...
-- Queue 3: ERRORS_Q — messages that will go to exception queue
BEGIN
    DBMS_AQADM.CREATE_QUEUE_TABLE(
        queue_table        => 'PDBADMIN.ERRORS_QT',
        queue_payload_type => 'PDBADMIN.AQ_TEST_PAYLOAD',
        multiple_consumers => TRUE
    );
EXCEPTION
...
BEGIN
    DBMS_AQADM.CREATE_QUEUE(
        queue_name  => 'PDBADMIN.ERRORS_Q',
        queue_table => 'PDBADMIN.ERRORS_QT',
        max_retries => 0,
        retry_delay => 0,
        retention_time => 86400
    );
    DBMS_AQADM.START_QUEUE(queue_name => 'PDBADMIN.ERRORS_Q');
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -24006 THEN RAISE; END IF;
END;
...
BEGIN
    DBMS_AQADM.ADD_SUBSCRIBER(
        queue_name => 'PDBADMIN.ERRORS_Q',
        subscriber => SYS.AQ$_AGENT('SUB_OK', NULL, NULL)
    );
EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
BEGIN
    DBMS_AQADM.ADD_SUBSCRIBER(
        queue_name => 'PDBADMIN.ERRORS_Q',
        subscriber => SYS.AQ$_AGENT('SUB_FAILING', NULL, NULL)
    );
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

-- Enqueue 10 messages to ERRORS_Q
DECLARE
    v_enqueue_options    DBMS_AQ.ENQUEUE_OPTIONS_T;
    v_message_properties DBMS_AQ.MESSAGE_PROPERTIES_T;
    v_message_handle     RAW(16);
...
BEGIN
    FOR i IN 1..10 LOOP
        v_payload := PDBADMIN.AQ_TEST_PAYLOAD(i, 'Error-prone message ' || i, 1, SYSTIMESTAMP);
        DBMS_AQ.ENQUEUE(
            queue_name         => 'PDBADMIN.ERRORS_Q',
            enqueue_options    => v_enqueue_options,
            message_properties => v_message_properties,
...
-- Dequeue and rollback for SUB_FAILING to trigger exception queue
DECLARE
    ...
    v_payload            PDBADMIN.AQ_TEST_PAYLOAD;
BEGIN
    v_dequeue_options.consumer_name := 'SUB_FAILING';
...
-- Queue 4: MISCONFIG_Q — dequeue disabled, long retention
BEGIN
    DBMS_AQADM.CREATE_QUEUE_TABLE(
        queue_table        => 'PDBADMIN.MISCONFIG_QT',
        queue_payload_type => 'PDBADMIN.AQ_TEST_PAYLOAD',
        multiple_consumers => TRUE
    );
EXCEPTION
...
BEGIN
    DBMS_AQADM.CREATE_QUEUE(
        queue_name  => 'PDBADMIN.MISCONFIG_Q',
        queue_table => 'PDBADMIN.MISCONFIG_QT',
        max_retries => 5,
        retry_delay => 10,
        retention_time => 0
    );
    DBMS_AQADM.START_QUEUE(queue_name => 'PDBADMIN.MISCONFIG_Q');
EXCEPTION
    WHEN OTHERS THEN
        IF SQLCODE != -24006 THEN RAISE; END IF;
END;
...
-- Disable dequeue (misconfiguration)
BEGIN
    DBMS_AQADM.STOP_QUEUE(queue_name => 'PDBADMIN.MISCONFIG_Q', dequeue => TRUE, enqueue => FALSE);
EXCEPTION WHEN OTHERS THEN NULL;
END;
/
...
BEGIN
    DBMS_AQADM.ADD_SUBSCRIBER(
        queue_name => 'PDBADMIN.MISCONFIG_Q',
        subscriber => SYS.AQ$_AGENT('SUB_STUCK', NULL, NULL)
    );
EXCEPTION WHEN OTHERS THEN NULL;
END;
/

-- Enqueue 5 stuck messages to MISCONFIG_Q
DECLARE
    v_enqueue_options    DBMS_AQ.ENQUEUE_OPTIONS_T;
    v_message_properties DBMS_AQ.MESSAGE_PROPERTIES_T;
    v_message_handle     RAW(16);
...
BEGIN
    FOR i IN 1..5 LOOP
        v_payload := PDBADMIN.AQ_TEST_PAYLOAD(i, 'Stuck message ' || i, 1, SYSTIMESTAMP);
        DBMS_AQ.ENQUEUE(
            queue_name         => 'PDBADMIN.MISCONFIG_Q',
            enqueue_options    => v_enqueue_options,
            message_properties => v_message_properties,
...
BEGIN
    ...
    DBMS_STATS.GATHER_TABLE_STATS('PDBADMIN', 'ERRORS_QT');
    DBMS_STATS.GATHER_TABLE_STATS('PDBADMIN', 'MISCONFIG_QT');
EXCEPTION WHEN OTHERS THEN NULL;
END;
/
